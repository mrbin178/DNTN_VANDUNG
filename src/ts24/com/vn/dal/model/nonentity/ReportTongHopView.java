package ts24.com.vn.dal.model.nonentity;

import java.io.Serializable;

@SuppressWarnings("serial")
public class ReportTongHopView implements Serializable {
	private String stt = "", maSoThue = "", mauHDon = "", soThuTu = "", kyHieuHDon = "", soHoaDon = "",
    ngayXHDon = "", hoTenNguoiMua = "", mstNguoiMua = "", tenHang = "", doanhSoBanChuaThue = "",
		thueSuat10 = "", thueSuat5 = "", thueSuat0 = "", thueSuatKCT = "", thueSuatKhongKeKhai = "",
		thueGTGT = "", chietKhauGiamTru = "", ghiChu ="";

	public String getStt() {
		return stt;
	}

	public void setStt(String stt) {
		this.stt = stt;
	}

	public String getMaSoThue() {
		return maSoThue;
	}

	public void setMaSoThue(String maSoThue) {
		this.maSoThue = maSoThue;
	}

	public String getMauHDon() {
		return mauHDon;
	}

	public void setMauHDon(String mauHDon) {
		this.mauHDon = mauHDon;
	}

	public String getSoThuTu() {
		return soThuTu;
	}

	public void setSoThuTu(String soThuTu) {
		this.soThuTu = soThuTu;
	}

	public String getKyHieuHDon() {
		return kyHieuHDon;
	}

	public void setKyHieuHDon(String kyHieuHDon) {
		this.kyHieuHDon = kyHieuHDon;
	}

	public String getSoHoaDon() {
		return soHoaDon;
	}

	public void setSoHoaDon(String soHoaDon) {
		this.soHoaDon = soHoaDon;
	}

	public String getNgayXHDon() {
		return ngayXHDon;
	}

	public void setNgayXHDon(String ngayXHDon) {
		this.ngayXHDon = ngayXHDon;
	}

	public String getHoTenNguoiMua() {
		return hoTenNguoiMua;
	}

	public void setHoTenNguoiMua(String hoTenNguoiMua) {
		this.hoTenNguoiMua = hoTenNguoiMua;
	}

	public String getMstNguoiMua() {
		return mstNguoiMua;
	}

	public void setMstNguoiMua(String mstNguoiMua) {
		this.mstNguoiMua = mstNguoiMua;
	}

	public String getTenHang() {
		return tenHang;
	}

	public void setTenHang(String tenHang) {
		this.tenHang = tenHang;
	}

	public String getDoanhSoBanChuaThue() {
		return doanhSoBanChuaThue;
	}

	public void setDoanhSoBanChuaThue(String doanhSoBanChuaThue) {
		this.doanhSoBanChuaThue = doanhSoBanChuaThue;
	}

	public String getThueSuat10() {
		return thueSuat10;
	}

	public void setThueSuat10(String thueSuat10) {
		this.thueSuat10 = thueSuat10;
	}

	public String getThueSuat5() {
		return thueSuat5;
	}

	public void setThueSuat5(String thueSuat5) {
		this.thueSuat5 = thueSuat5;
	}

	public String getThueSuat0() {
		return thueSuat0;
	}

	public void setThueSuat0(String thueSuat0) {
		this.thueSuat0 = thueSuat0;
	}

	public String getThueSuatKCT() {
		return thueSuatKCT;
	}

	public void setThueSuatKCT(String thueSuatKCT) {
		this.thueSuatKCT = thueSuatKCT;
	}

	public String getThueSuatKhongKeKhai() {
		return thueSuatKhongKeKhai;
	}

	public void setThueSuatKhongKeKhai(String thueSuatKhongKeKhai) {
		this.thueSuatKhongKeKhai = thueSuatKhongKeKhai;
	}

	public String getThueGTGT() {
		return thueGTGT;
	}

	public void setThueGTGT(String thueGTGT) {
		this.thueGTGT = thueGTGT;
	}

	public String getChietKhauGiamTru() {
		return chietKhauGiamTru;
	}

	public void setChietKhauGiamTru(String chietKhauGiamTru) {
		this.chietKhauGiamTru = chietKhauGiamTru;
	}

	public String getGhiChu() {
		return ghiChu;
	}

	public void setGhiChu(String ghiChu) {
		this.ghiChu = ghiChu;
	}

}
