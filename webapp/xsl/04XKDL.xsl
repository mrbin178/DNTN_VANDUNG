<?xml version="1.0" encoding="utf-8"?><!-- DWXMLSource="04_XKDL_DT.xml" -->
<!DOCTYPE xsl:stylesheet  [
	<!ENTITY nbsp   "&#160;">
	<!ENTITY copy   "&#169;">
	<!ENTITY reg    "&#174;">
	<!ENTITY trade  "&#8482;">
	<!ENTITY mdash  "&#8212;">
	<!ENTITY ldquo  "&#8220;">
	<!ENTITY rdquo  "&#8221;"> 
	<!ENTITY pound  "&#163;">
	<!ENTITY yen    "&#165;">
	<!ENTITY euro   "&#8364;">
]>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
<xsl:output method="html" encoding="utf-8" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
<xsl:decimal-format name="vndong" decimal-separator=',' grouping-separator='.' />
<xsl:variable name="lower">abcdefghijklmnopqrstuvwxyzàáảãạăằắẳẵặâầấẩẫậđèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵ</xsl:variable>
<xsl:variable name="upper">ABCDEFGHIJKLMNOPQRSTUVWXYZÀÁẢÃẠĂẰẮẲẴẶÂẦẤẨẪẬĐÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴ</xsl:variable>
<xsl:template match="/">

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>

<title>HOA_DON_DIEN_TU</title>

</head>

<body background="bgimage.jpg">
<div style="width:720px; border:1px solid #000; font-size:11pt; font-family:'Times New Roman', 'serif'; margin:0 auto;">

<xsl:choose>
  <xsl:when test="HDonDL/NDungHDon/TTinHDon/loaiHDonNopMau='1'">
<span style="position:absolute; z-index:100; margin:400px 0px 0px 250px;"><img src="data:image/png;base64,"/></span>

<!-- Mẫu số HĐ và Tiêu đề -->
<table width="700" align="center" style="color:BLUE">
    <tr valign="middle">
        <td width="200" rowspan="2" align="center">
            <xsl:choose>
            <xsl:when test="HDonDL/NDungHDon/TTinHDon/loaiLogo='00'"> </xsl:when>
            <xsl:when test="HDonDL/NDungHDon/TTinHDon/loaiLogo='01'"><img src='data:image/png;base64,{HDonDL/NDungHDon/TTinHDon/ddanLogo}' /></xsl:when>
            <xsl:otherwise> </xsl:otherwise>
            </xsl:choose>
        </td>
        <td align="center" valign="bottom" style="font-size:16pt">
            <b>PHIẾU XUẤT KHO HÀNG GỬI BÁN ĐẠI LÝ</b>
        </td>
        <td width="150" height="65" rowspan="2" align="left" valign="middle" style="font-size:9pt">
            Mẫu số: <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/mauHDon"/><br />
            Ký hiệu: <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/kyHieuHDon"/><br />
            Số: 
          <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/soHDon"/></td>
    </tr>
    <tr valign="middle">
      	<td align="center" valign="top">
        	<b>(HÓA ĐƠN ĐIỆN TỬ)</b><br />
    		<xsl:if test="HDonDL/NDungHDon/TTinHDon/lienHDon!=''"><xsl:value-of select="HDonDL/NDungHDon/TTinHDon/lienHDon"/></xsl:if>
            <xsl:if test="HDonDL/NDungHDon/TTinHDon/lienHDon=''"> </xsl:if>
    	</td>
	</tr>
</table><br />

<!-- Thông tin người bán -->
<table width="700" align="center" style="color:BLUE">
    <tr>
    <td colspan="3"><b><xsl:value-of select="HDonDL/NDungHDon/NBan/tenDVi"/></b></td>
    </tr>
  <tr>
    <td colspan="3">Địa chỉ: <xsl:value-of select="HDonDL/NDungHDon/NBan/diaChi"/></td>
  </tr>
  <tr>
    <td width="220">MST: <xsl:value-of select="HDonDL/NDungHDon/NBan/MST"/></td>
    <td>Tel: <xsl:value-of select="HDonDL/NDungHDon/NBan/dienThoai"/></td>
    <td width="220">Fax: <xsl:value-of select="HDonDL/NDungHDon/NBan/fax"/></td>
  </tr>  
</table>

<table width="700" align="center" style="color:BLUE">
  <tr>
    <td width="200">Số tài khoản: <xsl:value-of select="HDonDL/NDungHDon/NBan/soTKhoan"/></td>
    <td align="left">Tại ngân hàng: <xsl:value-of select="HDonDL/NDungHDon/NBan/taiNHang"/></td>
  </tr>  
  <tr>
    <td colspan="3" valign="middle">
        <hr style="border:1px solid #CCC"/>
    </td>
  </tr>
</table>

<!-- Thông tin người mua -->
<table width="700" align="center" style="color:BLUE">
  <tr>
    <td>Họ tên người vận chuyển: <xsl:value-of select="HDonDL/NDungHDon/NMua/hoTen"/></td>
    <td width="220">Hợp đồng số: <xsl:value-of select="HDonDL/NDungHDon/NMua/soHDong"/></td>
  </tr>
  <tr>
    <td colspan="2">Phương tiện vận chuyển: <xsl:value-of select="HDonDL/NDungHDon/NMua/phuongTienVChuyen"/></td>
  </tr>
  <tr>
    <td colspan="2">Nhập tại kho: <xsl:value-of select="HDonDL/NDungHDon/NMua/nhapTaiKho"/></td>
  </tr>
  <tr>
    <td colspan="2">Xuất tại kho: <xsl:value-of select="HDonDL/NDungHDon/NMua/xuatTaiKho"/></td>
  </tr>
  <tr>
    <td>Căn cứ hợp đồng kinh tế số: <xsl:value-of select="HDonDL/NDungHDon/NMua/canCuSoHDong"/></td>
    <td width="220">
    	<xsl:if test="HDonDL/NDungHDon/NMua/canCuNgayHDong=''">Ngày       tháng       năm      </xsl:if>
        <xsl:if test="HDonDL/NDungHDon/NMua/canCuNgayHDong!=''">Ngày <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,9,2)"/> tháng <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,6,2)"/> năm <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,1,4)"/></xsl:if>
    </td>
  </tr>
</table>

<!-- Dữ liệu chính trên HĐ -->
<table border="1" cellspacing="0" cellpadding="3" align="center" width="700" style="border-collapse:collapse;">
	<col title="STT" width="40"/>
    <col title="Tên hàng hóa" width="290"/>
    <col title="Mã số" width="50"/>
    <col title="Đơn vị tính" width="50"/>
    <col title="Số lượng" width="80"/>
    <col title="Đơn giá" width="80"/>
    <col title="Thành tiền" width="110"/>
  <tr>
    <td align="center" valign="middle"><b>STT</b></td>
    <td align="center" valign="middle"><b>Tên nhãn hiệu, quy cách, phẩm chất vật tư (Sản phẩm, hàng hóa)</b></td>
    <td align="center" valign="middle"><b>Mã số</b></td>
    <td align="center" valign="middle"><b>Đơn vị tính</b></td>
    <td align="center" valign="middle"><b>Số lượng</b></td>
    <td align="center" valign="middle"><b>Đơn giá</b></td>
    <td align="center" valign="middle"><b>Thành tiền (VND)</b></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td colspan="6" align="right" valign="middle"><b>Tổng cộng:</b></td>
    <td>&nbsp;</td>
  </tr>
</table>

<table width="700" align="center" style="color:BLUE">
  <tr>
    <td>Số tiền bằng chữ: 
    	<xsl:if test="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu=''"> </xsl:if>
        <xsl:if test="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu!=''"><xsl:value-of select="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu"/>.</xsl:if>        
    </td>
  </tr>
</table>

<!-- Thông tin chữ ký người mua và người bán -->
<table width="700" align="center" style="color:BLUE">
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
    <tr>
		<td colspan="5" align="right" valign="middle">
          	<i>Ngày <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,9,2)" /> tháng <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,6,2)" /> năm <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,1,4)" /></i><br />
        </td>
    </tr>
	<tr valign="middle">
		<td align="center" valign="bottom">
      		<b>Người nhận</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="bottom">
      		<b>Người lập</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="bottom">
      		<b>Thủ kho xuất</b><br />
            (Ký và ghi rõ họ tên)

        </td>
		<td align="center" valign="bottom">
      		<b>Người vận chuyển</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="middle">
          	<b>Thủ kho nhập</b><br />
            (Ký và ghi rõ họ tên)
        </td>   
    </tr>
</table>

<table width="700" align="center" style="color:BLUE">
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<tr valign="middle">
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiNhan"/>
        </td>
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiLap"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/thuKhoXuat"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiVanChuyen"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/thuKhoNhap"/>
        </td>   
    </tr>
</table>

<table width="700" align="center" style="color:BLUE">
	<tr>
		<td align="center" valign="top">
        	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/tieuDeChuyenHoaDonGiay"/>
            <br />
            <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/chuyenHoaDonGiay"/>
            <br /><br /><br />
			<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiChuyenDoi"/>
        </td>
    </tr>
	<tr>
    	<td align="center">
        	<xsl:if test="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe!=''"><xsl:value-of select="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe"/></xsl:if>
            <xsl:if test="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe=''"> </xsl:if>
        </td>
    </tr>
</table>

<!-- Ghi chú -->
<table width="700" align="center">
	<tr>
    	<td><hr style="border:1px solid #CCC"/></td>
    </tr>
</table>

<table width="700" align="center">
  <tr>
    <td width="20"> </td>
    <td align="justify" style="font-size:9pt">
    	<b><u> Ghi chú:</u></b>
        Có thể chuyển đổi từ hóa đơn điện tử sang hóa đơn giấy một lần duy nhất bằng cách vào phần mềm XuatHoaDon<sup>&#8482;</sup> của công ty TS24 chọn mục <b>chuyển đổi từ hóa đơn điện tử sang giấy</b> để có thể in ra giấy, ghi rõ họ tên, chữ ký của người chuyển đổi, đóng dấu xác nhận theo hướng dẫn của thông tư Hóa đơn điện tử.
    </td>
    <td width="20"> </td>
  </tr>
</table>
  </xsl:when>
  <xsl:otherwise>
<!-- Mẫu số HĐ và Tiêu đề -->
<table width="700" align="center" style="color:BLUE">
    <tr valign="middle">
        <td width="150" rowspan="2" align="center">
            <xsl:choose>
            <xsl:when test="HDonDL/NDungHDon/TTinHDon/loaiLogo='00'"> </xsl:when>
            <xsl:when test="HDonDL/NDungHDon/TTinHDon/loaiLogo='01'"><img src='data:image/png;base64,{HDonDL/NDungHDon/TTinHDon/ddanLogo}' /></xsl:when>
            <xsl:otherwise> </xsl:otherwise>
            </xsl:choose>
        </td>
        <td align="center" valign="bottom" style="font-size:16pt">
            <b>PHIẾU XUẤT KHO HÀNG GỬI BÁN ĐẠI LÝ</b>
        </td>
        <td width="150" height="65" rowspan="2" align="left" valign="middle" style="font-size:9pt">
            Mẫu số: <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/mauHDon"/><br />
            Ký hiệu: <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/kyHieuHDon"/><br />
            Số: 
          <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/soHDon"/></td>
    </tr>
    <tr valign="middle">
      	<td align="center" valign="top">
        	<b>(HÓA ĐƠN ĐIỆN TỬ)</b><br />
    		<xsl:if test="HDonDL/NDungHDon/TTinHDon/lienHDon!=''"><xsl:value-of select="HDonDL/NDungHDon/TTinHDon/lienHDon"/></xsl:if>
            <xsl:if test="HDonDL/NDungHDon/TTinHDon/lienHDon=''"> </xsl:if>
    	</td>
	</tr>
</table><br />

<!-- Thông tin người bán -->
<table width="700" align="center" style="color:BLUE">
    <tr>
    <td colspan="3"><b><xsl:value-of select="HDonDL/NDungHDon/NBan/tenDVi"/></b></td>
    </tr>
  <tr>
    <td colspan="3">Địa chỉ: <xsl:value-of select="HDonDL/NDungHDon/NBan/diaChi"/></td>
  </tr>
  <tr>
    <td width="220">MST: <xsl:value-of select="HDonDL/NDungHDon/NBan/MST"/></td>
    <td>Tel: <xsl:value-of select="HDonDL/NDungHDon/NBan/dienThoai"/></td>
    <td width="220">Fax: <xsl:value-of select="HDonDL/NDungHDon/NBan/fax"/></td>
  </tr>  
</table>

<table width="700" align="center" style="color:BLUE">
  <tr>
    <td width="200">Số tài khoản: <xsl:value-of select="HDonDL/NDungHDon/NBan/soTKhoan"/></td>
    <td align="left">Tại ngân hàng: <xsl:value-of select="HDonDL/NDungHDon/NBan/taiNHang"/></td>
  </tr>  
  <tr>
    <td colspan="3" valign="middle">
        <hr style="border:1px solid #CCC"/>
    </td>
  </tr>
</table>

<!-- Thông tin người mua -->
<table width="700" align="center" style="color:BLUE">
  <tr>
    <td>Họ tên người vận chuyển: <xsl:value-of select="HDonDL/NDungHDon/NMua/hoTen"/></td>
    <td width="220">Hợp đồng số: <xsl:value-of select="HDonDL/NDungHDon/NMua/soHDong"/></td>
  </tr>
  <tr>
    <td colspan="2">Phương tiện vận chuyển: <xsl:value-of select="HDonDL/NDungHDon/NMua/phuongTienVChuyen"/></td>
  </tr>
  <tr>
    <td colspan="2">Nhập tại kho: <xsl:value-of select="HDonDL/NDungHDon/NMua/nhapTaiKho"/></td>
  </tr>
  <tr>
    <td colspan="2">Xuất tại kho: <xsl:value-of select="HDonDL/NDungHDon/NMua/xuatTaiKho"/></td>
  </tr>
  <tr>
    <td>Căn cứ hợp đồng kinh tế số: <xsl:value-of select="HDonDL/NDungHDon/NMua/canCuSoHDong"/></td>
    <td width="220">
    	<xsl:if test="HDonDL/NDungHDon/NMua/canCuNgayHDong=''">Ngày       tháng       năm      </xsl:if>
        <xsl:if test="HDonDL/NDungHDon/NMua/canCuNgayHDong!=''">Ngày <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,9,2)"/> tháng <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,6,2)"/> năm <xsl:value-of select="substring(HDonDL/NDungHDon/NMua/canCuNgayHDong,1,4)"/></xsl:if>
    </td>
  </tr>
</table>

<!-- Dữ liệu chính trên HĐ -->
<table border="1" cellspacing="0" cellpadding="3" align="center" width="700" style="border-collapse:collapse;">
	<col title="STT" width="40"/>
    <col title="Tên hàng hóa" width="290"/>
    <col title="Mã số" width="50"/>
    <col title="Đơn vị tính" width="50"/>
    <col title="Số lượng" width="80"/>
    <col title="Đơn giá" width="80"/>
    <col title="Thành tiền" width="110"/>
  <tr>
    <td align="center" valign="middle"><b>STT</b></td>
    <td align="center" valign="middle"><b>Tên nhãn hiệu, quy cách, phẩm chất vật tư (Sản phẩm, hàng hóa)</b></td>
    <td align="center" valign="middle"><b>Mã số</b></td>
    <td align="center" valign="middle"><b>Đơn vị tính</b></td>
    <td align="center" valign="middle"><b>Số lượng</b></td>
    <td align="center" valign="middle"><b>Đơn giá</b></td>
    <td align="center" valign="middle"><b>Thành tiền (VND)</b></td>
  </tr>
  <xsl:for-each select="HDonDL/NDungHDon/DSachHHoaDVu/HHoaDVu">
  <tr>
    <td align="center" valign="middle"><xsl:value-of select="position()"/></td>
    <td align="left" valign="middle">
    	<xsl:choose>
        <xsl:when test="(tenHHoaDVu='') or not(tenHHoaDVu)"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="tenHHoaDVu"/></xsl:otherwise>
        </xsl:choose>
    </td>
    <td align="left" valign="middle">
    	<xsl:choose>
        <xsl:when test="(maSo='') or not(maSo)"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="maSo"/></xsl:otherwise>
        </xsl:choose>
    </td>
    <td align="center" valign="middle">
    	<xsl:choose>
        <xsl:when test="(DVT='') or not(DVT)"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="DVT"/></xsl:otherwise>
        </xsl:choose>
    </td>
    <td align="center" valign="middle">
    	<xsl:choose>
        <xsl:when test="(soLuong='') or not(soLuong)"> </xsl:when>
        <xsl:when test="soLuong='0'"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="format-number(soLuong, '#.##0,###', 'vndong')"/></xsl:otherwise>
        </xsl:choose>
    </td>
    <td align="right" valign="middle">
    	<xsl:choose>
        <xsl:when test="(donGia='') or not(donGia)"> </xsl:when>
        <xsl:when test="donGia='0'"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="format-number(donGia, '#.##0', 'vndong')"/></xsl:otherwise>
        </xsl:choose>
    </td>
    <td align="right" valign="middle">
    	<xsl:choose>
        <xsl:when test="(thanhTien='') or not(thanhTien)"> </xsl:when>
        <xsl:when test="thanhTien='0'"> </xsl:when>
        <xsl:otherwise><xsl:value-of select="format-number(thanhTien, '#.##0', 'vndong')"/></xsl:otherwise>
        </xsl:choose>
    </td>
  </tr>
  </xsl:for-each>
  <xsl:for-each select="HDonDL/NDungHDon/DSachHHoaDVu/TToan">
  <tr>
    <td colspan="6" align="right" valign="middle"><b>Tổng cộng:</b></td>
    <td align="right" valign="middle">
    	<xsl:choose>
        <xsl:when test="(tongTien='') or not(tongTien)"> </xsl:when>
        <xsl:when test="tongTien='0'"> </xsl:when>
        <xsl:otherwise><b><xsl:value-of select="format-number(tongTien, '#.##0', 'vndong')"/></b></xsl:otherwise>
        </xsl:choose>
    </td>
  </tr>
  </xsl:for-each>
</table>

<table width="700" align="center" style="color:BLUE">
  <tr>
    <td>Số tiền bằng chữ: 
    	<xsl:if test="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu=''"> </xsl:if>
        <xsl:if test="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu!=''"><xsl:value-of select="HDonDL/NDungHDon/DSachHHoaDVu/TToan/tongTienTToanBChu"/>.</xsl:if>        
    </td>
  </tr>
</table>

<!-- Thông tin chữ ký người mua và người bán -->
<table width="700" align="center" style="color:BLUE">
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
    <tr>
		<td colspan="5" align="right" valign="middle">
          	<i>Ngày <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,9,2)" /> tháng <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,6,2)" /> năm <xsl:value-of select="substring(HDonDL/NDungHDon/TTinHDon/ngayBan,1,4)" /></i><br />
        </td>
    </tr>
	<tr valign="middle">
		<td align="center" valign="bottom">
      		<b>Người nhận</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="bottom">
      		<b>Người lập</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="bottom">
      		<b>Thủ kho xuất</b><br />
            (Ký và ghi rõ họ tên)

        </td>
		<td align="center" valign="bottom">
      		<b>Người vận chuyển</b><br />
            (Ký và ghi rõ họ tên)
        </td>
		<td align="center" valign="middle">
          	<b>Thủ kho nhập</b><br />
            (Ký và ghi rõ họ tên)
        </td>   
    </tr>
</table>

<table width="700" align="center" style="color:BLUE">
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<col width="140" />
	<tr valign="middle">
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiNhan"/>
        </td>
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiLap"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/thuKhoXuat"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiVanChuyen"/>
        </td>   
		<td align="center" valign="middle">
          	<br /><br /><br /><br />
          	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/thuKhoNhap"/>
        </td>   
    </tr>
</table>

<table width="700" align="center" style="color:BLUE">
	<tr>
		<td align="center" valign="top">
        	<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/tieuDeChuyenHoaDonGiay"/>
            <br />
            <xsl:value-of select="HDonDL/NDungHDon/TTinHDon/chuyenHoaDonGiay"/>
            <br /><br /><br />
			<xsl:value-of select="HDonDL/NDungHDon/TTinHDon/nguoiChuyenDoi"/>
        </td>
    </tr>
	<tr>
    	<td align="center">
        	<xsl:if test="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe!=''"><xsl:value-of select="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe"/></xsl:if>
            <xsl:if test="HDonDL/NDungHDon/TTinHDon/hoaDonThayThe=''"> </xsl:if>
        </td>
    </tr>
</table>

<!-- Ghi chú -->
<table width="700" align="center">
	<tr>
    	<td><hr style="border:1px solid #CCC"/></td>
    </tr>
</table>

<table width="700" align="center">
  <tr>
    <td width="20"> </td>
    <td align="justify" style="font-size:9pt">
    	<b><u> Ghi chú:</u></b>
        Có thể chuyển đổi từ hóa đơn điện tử sang hóa đơn giấy một lần duy nhất bằng cách vào phần mềm XuatHoaDon<sup>&#8482;</sup> của công ty TS24 chọn mục <b>chuyển đổi từ hóa đơn điện tử sang giấy</b> để có thể in ra giấy, ghi rõ họ tên, chữ ký của người chuyển đổi, đóng dấu xác nhận theo hướng dẫn của thông tư Hóa đơn điện tử.
    </td>
    <td width="20"> </td>
  </tr>
</table>
  </xsl:otherwise>
</xsl:choose>

</div>
</body>
</html>

</xsl:template>
</xsl:stylesheet>