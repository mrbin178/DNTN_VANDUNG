<?xml version="1.0" encoding="utf-8"?><!-- DWXMLSource="HDDT_GTGT_Crown_M2.xml" -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:inv="http://laphoadon.gdt.gov.vn/2014/09/invoicexml/v1">
  ...
  <xsl:template match="inv:invoice">
    <html>
      ...
      <body>
        ...
        -->
      <div style="width:700px; margin:0 auto; padding:0px; text-align:center; border:1px solid #0a51a1;">
        <!-- Background -->
        ...
        <xsl:choose>
          ...
          <xsl:when test="(inv:invoiceData/inv:invoiceNumber) and (inv:invoiceData/inv:invoiceNumber)='0000000'">
            <table cellspacing="0" cellpadding="2" style="table-layout:fixed; border:3px double #0a51a1;">
              <col width="6%"/>
              <col width="37%"/>
              <col width="10%"/>
              <col width="12%"/>
                ...
                <xsl:when test="not(/inv:invoice/inv:invoiceData/inv:currencyCode) or (/inv:invoice/inv:invoiceData/inv:currencyCode)='' or translate(/inv:invoice/inv:invoiceData/inv:currencyCode,$lower,$upper)='VND' or translate(/inv:invoice/inv:invoiceData/inv:currencyCode,$lower,$upper)='VNĐ'">
                  <tr>
                    ...
                    <td align="left" class="borderTD">
                      <xsl:choose>
                        <xsl:when test="(/inv:invoice/inv:invoiceData/inv:userDefines/inv:ThongTinHangChung) and (/inv:invoice/inv:invoiceData/inv:userDefines/inv:ThongTinHangChung)!=''"><xsl:value-of select="/inv:invoice/inv:invoiceData/inv:userDefines/inv:ThongTinHangChung"/></xsl:when>
                        <xsl:otherwise>&nbsp;</xsl:otherwise>
                      </xsl:choose>
                    </td>
                    <td class="borderTD">&nbsp;</td>
      ...
      <div style="width:700px; margin:0 auto; padding:0px; text-align:center;">
        <!-- Thông tin Quảng cáo -->
        <xsl:copy-of select="$QuangCao"/>
      </div>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:variable name="Background">
    <xsl:choose>
      <xsl:when test="(/inv:invoice/inv:invoiceData/inv:userDefines/inv:Background) and  (/inv:invoice/inv:invoiceData/inv:userDefines/inv:Background!='')"><div style="position:absolute; z-index:-1; text-align:center; vertical-align:middle; margin:350px 0px 0px 0px; opacity:1.0; filter: alpha(opacity=100);"><img style="margin:105px 0px 0px 0px;" src='data:image/png;base64,{/inv:invoice/inv:invoiceData/inv:userDefines/inv:Background}' /></div></xsl:when>
      <xsl:otherwise></xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  ...
  <xsl:variable name="TTNguoiBan">
    <table style="table-layout:auto;">
      ...
      <col width="35%" />
      <tr>
        <td colspan="4" align="left" valign="top" style="padding-right:7px; border-top:1px solid #0a51a1;">
          <span style="font-size:14pt;"><b><xsl:value-of select="translate(/inv:invoice/inv:invoiceData/inv:sellerLegalName,$lower,$upper)"/></b></span>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="2" align="left" valign="top">Tên tài khoản <i>(Account Name)</i>: Crown Worldwide Ltd.</td>
        <td colspan="2" align="left" valign="top">Tại <i>(At)</i>: <xsl:value-of select="/inv:invoice/inv:invoiceData/inv:sellerBankName"/></td>
      </tr>
      <tr>
        <td colspan="4" align="left" valign="top">
          Số tài khoản <i>(Account No.)</i>: (USD) 001-067461-001
  ...
  <xsl:variable name="TTXacThucvaTTGiaoDich">
    <table>
      ...
      <tr>
        ...
        <td colspan="3" align="left" valign="top">Email: <xsl:value-of select="/inv:invoice/inv:invoiceData/inv:buyerEmail"/></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="3" align="left" valign="top">Fax: <xsl:value-of select="/inv:invoice/inv:invoiceData/inv:buyerFaxNumber"/></td>
      </tr>
      <tr>
  ...
  <!-- Đưa vào Quảng cáo -->
  <xsl:variable name="QuangCao">
    <table style="table-layout:auto">
      <col width="50%" />
      <col width="50%" />
      <tr>
        <td align="left">
          <b>Công ty TNHH Crown Worldwide - CN Hà Nội</b><br />
          Đc: Tầng 5, số 9 Xuân Diệu, Q.Tây Hồ, Hà Nội
        </td>
        <td align="left" style="white-space:nowrap">
          <b>Công ty TNHH Crown Worldwide - CN Đà Nẵng</b><br />
          Đc: Đường số 2, KCN Hòa Cầm, P. Hòa Thọ Tây, Q. Cẩm Lệ, TP Đà Nẵng
        </td>
      </tr>
    </table>
  </xsl:variable>
