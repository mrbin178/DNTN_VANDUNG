<?xml version="1.0" encoding="utf-8"?><!-- DWXMLSource="HDDT_GTGT_M10.xml" -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:inv="http://laphoadon.gdt.gov.vn/2014/09/invoicexml/v1">
              ...
              <xsl:choose>
                ...
                <xsl:otherwise>
                  ...
                  <tr>
                    <td align="center" class="borderTD_detail">
                      <xsl:choose>
                        <xsl:when test="(inv:lineNumber) and (inv:lineNumber)!='' and (inv:lineNumber)!=0"><xsl:value-of select="format-number(inv:lineNumber, '#,##0.##', 'usd')"/></xsl:when>
                        <xsl:otherwise>&nbsp;</xsl:otherwise>
                      </xsl:choose>
                    </td>
                    <td align="left" class="borderTD">
  ...
  <xsl:variable name="TTNguoiBan">
    <table style="table-layout:auto;">
      ...
      <col width="40%" />
      <tr>
        <td rowspan="7" align="left" valign="middle" style="max-width:200px; max-height:150px; white-space:nowrap; padding-right:7px; border-bottom: 1px solid #0a51a1;">
          <xsl:choose>
      ...
      <tr>
        <td colspan="2" align="left" valign="top" style="border-bottom: 1px solid #0a51a1;">Tại ngân hàng <i>(At)</i>: <xsl:value-of select="/inv:invoice/inv:invoiceData/inv:sellerBankName"/></td>
      </tr>
    </table>
  </xsl:variable>
    ...
    <table cellspacing="0" cellpadding="2" style="table-layout:fixed;">
      ...
      <xsl:choose>
        <xsl:when test="not(/inv:invoice/inv:invoiceData/inv:currencyCode) or (/inv:invoice/inv:invoiceData/inv:currencyCode)='' or translate(/inv:invoice/inv:invoiceData/inv:currencyCode,$lower,$upper)='VND' or translate(/inv:invoice/inv:invoiceData/inv:currencyCode,$lower,$upper)='VNĐ'">
          <xsl:for-each select="/inv:invoice/inv:invoiceData/inv:items/inv:item">
          <tr>
            <td align="center" class="borderTD_detail"><xsl:number format="1"/></td>
            <td align="left" class="borderTD">
              <xsl:choose>
                <xsl:when test="(inv:itemName) and (inv:itemName)!=''"><xsl:value-of select="inv:itemName"/></xsl:when>
                <xsl:otherwise>&nbsp;</xsl:otherwise>
